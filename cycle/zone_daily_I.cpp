				  ...
				  struct 	command_line_object *command_line, struct 	date 	current_date)
{
	...
	//=====================================================================================================================================
	double	trans_coeff1, z_delta, es;
	...
	//=====================================================================================================================================
		...
		//cout << patch->patch_defaults->max_snow_temp << endl;
		//cout << patch->metv.tavg << " " << patch->metv.tmin << " " << patch->metv.tday <<" "<<patch->metv.tnight<< endl;
		//getchar();
	...
	if (patch->daily_clim->tsoil != -999.){
		...
		//	patch->metv.tsoil_sum = 0.95*patch->metv.tsoil_sum + 0.05*(0.75 *patch->metv.tmin + 0.25 *patch->metv.tmax);
		...
		patch->tsoil10[20] = 0.55 *patch->metv.tmin + 0.45 *patch->metv.tmax;
		patch->metv.tsoil = patch->tsoil10[0] / 21.;

		for (int jj = 1; jj<21; jj++){
			patch->metv.tsoil += patch->tsoil10[jj] / 21.;
			patch->tsoil10[jj - 1] = patch->tsoil10[jj];
		}
	}
