...
Farquhar photosynthesis routine
	...
	Ko = Ko25 * pow(q10Ko, (t-15.0)/10.0);
	Ko = Ko * 100.0;   /* mbar --> Pa */
	
	/////////////////////////////////////////////////////////////////
	// uncommented by guoping
	//if (t > 15.0){
	...
	/* New, substituting our vbl names (note I'm assuming that
        ...
        */
        ...
        J =5.8* (-bb - sqrt(bb*bb - 4.0*aa*cc))/(2.0*aa); //changed by guoping
	...
	Av = max((-bb + sqrt(det)) / (2.0*aa),0.); //changed by guoping
	...
	Aj =max( (-bb + sqrt(det)) / (2.0*aa),0.); //changed by guoping
