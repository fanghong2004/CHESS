						 ...
						 double	Lstar_snow)
{
	...
	if (T_air < 0.0 && rain==0. && snow==0.) {
		...
		snowpack->sublimation = min(snowpack->sublimation, 0.00005);
	...
	//snowpack->snowmelt = T_melt;
	//cout << melt_Tcoef << " " << snowpack->overstory_fraction << endl;
	//getchar();
	//printf(" snowpack over story fraction is  %f %f\n", melt_Tcoef, snowpack->overstory_fraction);
	//getchar();
	//if (current_date.year == 2015 && current_date.month == 1 && current_date.day == 15){
		//cout << T_air << " " << snowpack->energy_deficit << endl;
		//cout << T_melt <<" "<< maximum_energy_deficit<<" "<<pre_energy_deficit<< endl;
		//getchar();
	//}
